...
class Ball:
    ...
    def move(self, player1, player2):
        ...
        if self.rect.colliderect(player1.rect):
            self.direction_x = 1  # 공을 오른쪽으로 반사
            self.rect.left = player1.rect.right  # 라켓 경계 밖으로 공 이동

        elif self.rect.colliderect(player2.rect):
            self.direction_x = -1  # 공을 왼쪽으로 반사
            self.rect.right = player2.rect.left  # 라켓 경계 밖으로 공 이동
