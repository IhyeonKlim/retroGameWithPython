...
class Gun:
    ...
    def __init__(self):
        ...
        #  머즐 플래시 관련 변수
        self.muzzle_flash_image = None
        self.muzzle_flash_time = None  # 머즐 플래시 활성화 시간
        self.muzzle_flash_duration = 100  # 100ms 동안만 표시

        # 머즐 플래시 이미지 로드
        muzzle_flash_path = "hit_effect.png"  # 피격 효과 이미지 재사용
        if os.path.exists(muzzle_flash_path):
            self.muzzle_flash_image = pygame.image.load(muzzle_flash_path)
            self.muzzle_flash_image = pygame.transform.scale(self.muzzle_flash_image, (80, 80))  # 크기 조정
        else:
            print(f"Warning: '{muzzle_flash_path}' not found. Muzzle flash disabled.")
    ...
    def shoot(self):
        ...
        if self.bullets > 0 and not self.isFullReloading:
            ...
            #  머즐 플래시 효과 활성화
            self.muzzle_flash_time = pygame.time.get_ticks()
    ...
    def draw_muzzle_flash(self, position):
        """ 머즐 플래시를 총구 위치에 표시"""
        if self.muzzle_flash_image and self.muzzle_flash_time:
            current_time = pygame.time.get_ticks()
            if current_time - self.muzzle_flash_time < self.muzzle_flash_duration:
                # 총구 위치에 플래시 이미지 표시
                screen.blit(self.muzzle_flash_image, (position[0] - 40, position[1] - 40))
            else:
                self.muzzle_flash_time = None  # 일정 시간이 지나면 플래시 제거
...
class Enemy:
    ...
    def draw(self):
        """적을 화면에 그리기"""
        if self.image:
            screen.blit(self.image, self.rect.topleft)  # 이미지 표시
        else:
            pygame.draw.rect(screen, RED, self.rect)  # 기본 빨간 사각형
...
class Game:
    ...
    def __init__(self):
        ...
        # 피격 효과 이미지 로드
        self.hit_image = None
        hit_image_path = "hit_effect.png"
        if os.path.exists(hit_image_path):
            self.hit_image = pygame.image.load(hit_image_path)
            self.hit_image = pygame.transform.scale(self.hit_image, (WIDTH, HEIGHT))
        else:
            print(f"Warning: '{hit_image_path}' not found. Using default red overlay.")

    def display_hit_effect(self):
        """피격 효과를 화면에 표시"""
        if self.hit_effect_time and pygame.time.get_ticks() - self.hit_effect_time < 300:
            if self.hit_image:
                screen.blit(self.hit_image, (0, 0))  # 피격 이미지 표시
            # 이미지가 있던 없던 빨간색 화면 표시
    ...
    def draw_background(self):
        ...
        if self.background_image:
            screen.blit(self.background_image, (0, 0))  # 배경 이미지 표시
        else:
            screen.fill(BLACK)  # 배경이 없으면 기본 검은색 배경
    ...
    def draw_objects(self):
        ...
        self.gun.draw_muzzle_flash(mouse_pos)
        ...
        if self.hit_effect_time and pygame.time.get_ticks() - self.hit_effect_time < self.hit_effect_duration:
            self.display_hit_effect() # 피격 효과 이미지 추가
