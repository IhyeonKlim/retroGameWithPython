...
class Player:
    """플레이어 클래스"""
    def __init__(self):
        self.size = 30  # 플레이어 크기 설정
        self.speed = 4  # 플레이어 속도 설정
        # 플레이어 위치 초기화 (화면 중앙에서 시작)
        self.rect = pygame.Rect(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2, self.size, self.size)

        # 이미지 조정을 위한 함수 (원본 이미지가 손상되지 않도록)
        def scale_image(image, target_size):
            """이미지의 원본 비율을 유지하면서 크기를 조절"""
            image_rect = image.get_rect()
            original_width, original_height = image_rect.size
            target_width, target_height = target_size

            # 가로, 세로 비율 중 작은 값을 기준으로 크기 조절
            scale_factor = min(target_width / original_width, target_height / original_height)
            new_size = (int(original_width * scale_factor), int(original_height * scale_factor))

            return pygame.transform.smoothscale(image, new_size)  # 부드러운 크기 조정

        try:
            # 이미지 로드 및 비율 유지 크기 조정
            original_image = pygame.image.load("spaceship.png")
            scaled_image = scale_image(original_image, (self.size, self.size))

            # 90도 회전 후 다시 비율 유지하면서 크기 조정
            angle = 90 # 90도 회전을 위한
            rotated_image = pygame.transform.rotate(scaled_image, 90)
            self.image = scale_image(rotated_image, (self.size, self.size))

        except pygame.error:
            print("플레이어 이미지를 불러올 수 없습니다! 기본 사각형을 사용합니다.")
            self.image = None  # 이미지가 없을 경우 대비
...
class Game:
    ...
    def __init__(self):
        ...
        self.music_available = True  # 음악 없음 플래그 설정

        # BGM 불러오기
        pygame.mixer.init()

        try:
            pygame.mixer.music.load("Orbital_Colossus.mp3")
            # 음악 볼륨 설정
            pygame.mixer.music.set_volume(0.5)  # 볼륨 50% (0.0 ~ 1.0)
        except pygame.error:
            print("배경 음악을 불러올 수 없습니다. 음악 없이 게임을 실행합니다.")
            self.music_available = False  # 음악 없음 플래그 설정
    ...
    def play_game(self):
        ...
        # 음악이 존재하며 재생 중이 아닐 때만 실행
        if self.music_available and not pygame.mixer.music.get_busy():
