...
class Player:
    def __init__(self, x, y, width, height, speed, color, bullet_interval=200, image_path=None):
        ...
        # 이미지 관련 수정
        self.original_image = None  # 원본 이미지를 저장할 변수 추가
        self.image = None

        if image_path:
            self.original_image = pygame.image.load(image_path)
            self.original_image = pygame.transform.scale(self.original_image, (width, height))
            self.image = self.original_image.copy()  # 원본 이미지의 복사본으로 초기 이미지 설정
        ...
        self.bullet_count = 1  # 발사할 총알 개수
        self.bullet_level = 1  # 총알 레벨 (영구지속)
    ...
    def move(self, keys, screen_width):
        """플레이어의 키 입력에 따라 이동 및 하이퍼 플라이트 시 중앙으로 이동"""

        # 하이퍼 플라이트가 활성화된 경우
        if self.hyper_flight_active:
            # 플레이어의 x 좌표를 화면 중앙으로 이동
            self.y = (screen_width - self.height) // 0.5
            if keys[pygame.K_LEFT] and self.x > 0:
                self.x -= self.speed * 3
            if keys[pygame.K_RIGHT] and self.x < screen_width - self.width:
                self.x += self.speed * 3

        # 하이퍼 플라이트가 비활성화된 경우 (기존 이동 로직)
        else:
    ...
    # 아이템 + 하이퍼 플라이트
    def activate_hyper_flight(self, ticks_manager, duration=2000):
        """하이퍼 플라이트 활성화"""
        if not self.hyper_flight_active:  # 이미 활성화된 경우 크기 증가 방지
            # 원본 이미지(self.original_image)를 2배 크기로 스케일링
            self.width *= 2
            self.height *= 2
    ...
    def deactivate_hyper_flight(self):
        ...
        # 크기를 원래대로 되돌리고, 원본 이미지(self.original_image)로 복구
        self.width //= 2  # 크기 복원
        self.height //= 2  # 크기 복원
