...
class Monster:
    ...
    @staticmethod
    def handle_hyper_flight_collision(monsters, mid_screen_height, score, coin_list, item_list, item_spawn_chance=10):
        """
        하이퍼 플라이트 상태에서 화면 중간 높이 이하의 몬스터를 충돌 처리.
        - 하이퍼 플라이트 상태일 때 아이템 드랍 확률을 조정해야 함.
        - 하이퍼 플라이트가 '계속' 나오지 않도록 조정
        - mid_screen_height: 화면 중간 높이
        - item_spawn_chance: 아이템 생성 확률 (기본값 10%)
        """
        for monster in monsters[:]:
